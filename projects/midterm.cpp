...
int main() {
	...
	cout<<"Shifting integar K = 42"<<endl;
	input.K=42;
...
char encode(char input_char, int K){
	...
	for (int i=0;i<26;i++) {
		...
		if(65<=input_char && input_char<=90){//Upper case
			if((input_char-65+K)%26<0){
				output_char=65+26+(input_char-65+K)%26;
			}
			else{
		...
		else if(97<=input_char && input_char<=122){//lower case
			if((input_char-97+K)%26<0){
				output_char=97+26+(input_char-97+K)%26;
			}
			else{
		...
		else if(47<input_char && input_char<58){//if char is from 0-9
			output_char=input_char-10;           //Please check one https://www.learncpp.com/cpp-tutorial/chars/
			return output_char;
			break;
		}

		else if(37<input_char && input_char<48){//if char is from 0-9
			output_char=input_char+10;           //Please check one https://www.learncpp.com/cpp-tutorial/chars/
			return output_char;
			break;
		}
